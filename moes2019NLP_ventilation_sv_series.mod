...
var masswater{Time} >= 0.001;
var Trelease2 >=0, <= 20; #outlet temperature after air-water heat pump evaporator
var Tinter{Time} <= EPFLMediumT, >= EPFLMediumOut;
################################
# Constraints
#######
...
subject to mCp{t in Time}: #DTmin condition on the other side of the HEX;
masswater[t] = Qheating[t]/(EPFLMediumT - EPFLMediumOut);
...
COP2[t] = CarnotEff*(TLMCond2[t]/(TLMCond2[t] - TLMEvapHP2[t]));

subject to dTLMCondensor{t in Time}: #the logarithmic mean temperature in the condenser. Note: should be in K
#TLMCond[t] = (EPFLMediumT - Tinter[t])/log((EPFLMediumT + 273)/(Tinter[t] + 273));
TLMCond[t] = ((((Tinter[t]+273)*(EPFLMediumT+273)^2) + ((EPFLMediumT+273)*(Tinter[t]+273)^2))/2)^(1/3);

subject to dTLMCondensor2{t in Time}: #the logarithmic mean temperature in the condenser for air-water heat pump. Note: should be in K
#TLMCond2[t] = (Tinter[t] - EPFLMediumOut)/log((Tinter[t] + 273)/(EPFLMediumOut + 273));
TLMCond2[t] = ((((Tinter[t]+273)*(EPFLMediumOut+273)^2) + ((EPFLMediumOut+273)*(Tinter[t]+273)^2))/2)^(1/3);
...
subject to waterHeatBalance1{t in Time}: #the heat demand of EPFL should be the sum of the heat delivered by the 2 systems;
Qcond[t] = masswater[t]*(EPFLMediumT - Tinter[t]);

subject to waterHeatBalance2{t in Time}: #the heat demand of EPFL should be the sum of the heat delivered by the 2 systems;
Qcond2[t] = masswater[t]*(Tinter[t] - EPFLMediumOut);
